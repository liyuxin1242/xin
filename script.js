...
function initializeStats() {
    ...
    // 设置初始点赞数
    likeNumber.textContent = currentStats.totalLikes;
    ...
    likeButton.addEventListener('click', async function() {
        if (currentStats.canLikeToday) {
            ...
            // 尝试处理点赞
            const success = await stats.handleLike();
            ...
            if (success) {
                ...
                // 更新点赞数显示
                likeNumber.textContent = newStats.totalLikes;
                ...
                // 添加点赞成功的动画效果
                const likeNumberElement = document.getElementById('like-number');
                likeNumberElement.classList.add('number-animation');
                setTimeout(() => {
                    likeNumberElement.classList.remove('number-animation');
                }, 1000);
            }
        } else {
            // 显示已点赞提示
